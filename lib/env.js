...
export function setupEnvironment(requiredVars = [], defaultValues = {}) {
    ...
    if (requiredVars.includes('wpSitemap')) {
        let wpSitemap = __ENV.WPSITEMAP || defaultValues.wpSitemap || 'wp-sitemap.xml'; // default
        envVars.wpSitemap = wpSitemap;
    }
    ...
    // Handle UserAgent - always check, no requiredVars needed
    if (__ENV.USERAGENT || defaultValues.userAgent) {
        envVars.userAgent = __ENV.USERAGENT || defaultValues.userAgent;
    }
