 ...
 *   @param Set      assets       - Set of asset URLs (for O(1) lookups).
...
export function findNewAssets(response, assets, domainFilter) {
    ...
    //find assets not already loaded (in our assets Set) - .has() is O(1) vs .includes() O(n)
    let newAssets = findAssets(doc).filter(x => !assets.has(x))
...
/*
 *   Summary.     getAssetGroupName
 *   Description. Returns a grouped name for an asset URL based on file extension.
 *                This reduces InfluxDB cardinality by grouping thousands of unique
 *                asset URLs (with query params like ?ver=123) into a few categories.
 *
 *   @param  string url        - asset URL
 *   @return string groupName  - grouped name (e.g., 'asset-js', 'asset-css', 'asset-image')
 */
export function getAssetGroupName(url) {
 ...
 *   Description. Creates an array for batch k6 calls. Assets are tagged with a
 *                grouped name based on file type to reduce metric cardinality.
 *
...
export function createBatchArrayFromURLArray (urls,method,body=null,params=null) {
    ...
    if (urls.length > 0) {
        batchArray = urls.map(url => {
            // Group assets by type to reduce InfluxDB cardinality
            const groupedName = getAssetGroupName(url);
            const taggedParams = {
                ...params,
                tags: {
                    ...(params?.tags || {}),
...
export function getPageAssets(response,data, metrics) {
    ...
    if(newAssets.length > 0){
        ...
        newAssets.forEach(asset => data.assets.add(asset))
...
export function addToCart(url,sku,id,quantity, data, metrics) {
    ...
    const customParams = {
        ...data.params,
        headers: { ...defaultHeaders, ...(data.params.headers || {}) }
    };
    const response = http.post(
        url,
        {
