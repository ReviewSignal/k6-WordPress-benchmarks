...
export function getPage(url, data){ 
    ...
    check(response, isOK) || addErrorMetrics()
    addResponseMetrics(response)

    //load page assets
    getPageAssets(response,data)

    //in case you want to do anything further with response
    return response
}

export function getPageAssets(response,data) {
    //load all secondary assets
    ...
    if(newAssets.length > 0){
        ...
        for (let key in pageAssetResponses) {
            check(pageAssetResponses[key], isOK) || addErrorMetrics()
...
export function addToCart(url,sku,id,quantity, data) {
    const defaultHeaders = {
                "content-type":
                "application/x-www-form-urlencoded;type=content-type;mimeType=application/x-www-form-urlencoded",
            };
